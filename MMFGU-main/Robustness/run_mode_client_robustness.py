...
from MoDe.config import Config
from Robustness.mode_runner import RobustnessMoDeServer
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run MoDe client-unlearning robustness sweep")
...
def build_pretrained_snapshot(server: RobustnessMoDeServer) -> dict[str, object]:
...
def restore_server_from_snapshot(config: Config, snapshot: dict[str, object]) -> RobustnessMoDeServer:
    server = RobustnessMoDeServer(config)
...
def main() -> None:
    ...
    print("Running MoDe robustness configuration:")
    print(json.dumps({
        "data_dir": config.data_dir,
        "num_clients": config.num_clients,
        "federated_rounds": config.federated_rounds,
        "local_epochs": config.local_epochs,
        "momentum_coeff": config.momentum_coeff,
        "degradation_rounds": config.degradation_rounds,
        "guidance_rounds": config.guidance_rounds,
    ...
    with open(output_dir / "mode_robustness_summary.json", "w", encoding="utf-8") as file:
    ...
    print(f"Saved summary to: {output_dir / 'mode_robustness_summary.json'}")
