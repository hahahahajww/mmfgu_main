...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run MMFGU robustness across multiple seeds")
...
def build_command(args: argparse.Namespace, seed: int, output_dir: Path) -> list[str]:
    return [
        args.python,
        str(Path(__file__).resolve().parent / "run_mmfgu_client_robustness.py"),
        "--data-dir",
        args.data_dir,
        "--seed",
        str(seed),
...
def main() -> None:
    ...
    for seed in args.seeds:
        ...
        print(f"\n=== Running robustness seed={seed} ===")
        subprocess.run(build_command(args, seed, run_dir), check=True)
        with open(run_dir / "robustness_summary.json", "r", encoding="utf-8") as file:
    ...
    for ratio in ratios:
        ...
        affected_count = [row["affected_client_count"] for row in matched_runs]
        ratio_rows.append(
            {
                ...
                "affected_client_count": metric_mean_std(affected_count),
            }
        )
    ...
    print("\n=== Robustness Mean ± Std Summary ===")
    for row in ratio_rows:
        print(
            f"ratio={row['client_unlearn_ratio']:.2f} after_test={row['after_test']['mean']*100:.2f} ± {row['after_test']['std']*100:.2f} affected={row['affected_client_count']['mean']:.2f}"
        )
