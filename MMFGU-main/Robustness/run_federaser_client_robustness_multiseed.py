...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run FedEraser robustness across multiple seeds"
    )
...
def build_command(args: argparse.Namespace, seed: int, output_dir: Path) -> list[str]:
    return [
        args.python,
        str(Path(__file__).resolve().parent / "run_federaser_client_robustness.py"),
        "--data-dir",
        args.data_dir,
        "--task",
        "node_classification",
...
def main() -> None:
    ...
    for seed in args.seeds:
        ...
        print(f"\n=== Running FedEraser robustness seed={seed} ===")
        subprocess.run(build_command(args, seed, run_dir), check=True)
        with open(run_dir / "fed_eraser_robustness_summary.json", "r", encoding="utf-8") as file:
    ...
    for ratio in ratios:
        ...
        reconstruction_seconds = [row["reconstruction_seconds"] for row in matched_runs]
        ratio_rows.append(
            {
                ...
                "reconstruction_seconds": metric_mean_std(reconstruction_seconds),
            }
        )
    ...
    with open(output_root / "fed_eraser_summary.json", "w", encoding="utf-8") as file:
        json.dump(summary, file, indent=2)

    print("\n=== FedEraser Robustness Mean ± Std Summary ===")
    for row in ratio_rows:
        print(
            f"ratio={row['client_unlearn_ratio']:.2f} after_test={row['after_test']['mean']*100:.2f} ± {row['after_test']['std']*100:.2f}"
        )
    print(f"Saved summary to: {output_root / 'fed_eraser_summary.json'}")
