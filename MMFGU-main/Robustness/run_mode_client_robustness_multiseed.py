...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run MoDe robustness across multiple seeds")
...
def build_command(args: argparse.Namespace, seed: int, output_dir: Path) -> list[str]:
    return [
        args.python,
        str(Path(__file__).resolve().parent / "run_mode_client_robustness.py"),
        ...
        "--momentum-coeff", str(args.momentum_coeff),
        "--degradation-rounds", str(args.degradation_rounds),
        "--guidance-rounds", str(args.guidance_rounds),
...
def main() -> None:
    ...
    for seed in args.seeds:
        ...
        print(f"\n=== Running MoDe robustness seed={seed} ===")
        subprocess.run(build_command(args, seed, run_dir), check=True)
        with open(run_dir / "mode_robustness_summary.json", "r", encoding="utf-8") as file:
    ...
    with open(output_root / "mode_summary.json", "w", encoding="utf-8") as file:
        json.dump(summary, file, indent=2)

    print("\n=== MoDe Robustness Mean ± Std Summary ===")
    for row in ratio_rows:
        print(f"ratio={row['client_unlearn_ratio']:.2f} after_test={row['after_test']['mean']*100:.2f} ± {row['after_test']['std']*100:.2f}")
    print(f"Saved summary to: {output_root / 'mode_summary.json'}")
