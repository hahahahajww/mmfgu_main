...
MODE_TO_ENTRYPOINT = {
    "unlearning": "FedKD.run_fedkd",
    "retrain": "FedKD.run_fedkd_retrain",
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run FedKD across multiple seeds")
    parser.add_argument("--data-dir", required=True)
    parser.add_argument("--seeds", nargs="+", type=int, required=True)
    parser.add_argument("--mode", choices=list(MODE_TO_ENTRYPOINT), default="unlearning")
    parser.add_argument("--python", default=sys.executable)
    parser.add_argument("--device", default="cuda")
    parser.add_argument("--output-root", default="E:\\MMFGU\\FedKD\\outputs_multiseed")
...
def summarize_runs(rows: list[dict], mode: str) -> dict:
    ...
    if mode == "unlearning":
        before_val = [row["fedkd"]["before_global_metrics"][avg_val_key] for row in rows]
        before_test = [row["fedkd"]["before_global_metrics"][avg_test_key] for row in rows]
        after_val = [row["fedkd"]["after_global_metrics"][avg_val_key] for row in rows]
        after_test = [row["fedkd"]["after_global_metrics"][avg_test_key] for row in rows]
        delta_val = [row["fedkd"]["metric_delta"][f"{avg_val_key}_delta"] for row in rows]
        delta_test = [row["fedkd"]["metric_delta"][f"{avg_test_key}_delta"] for row in rows]
        result["fedkd"] = {
